...
class GameState:
    ...
    def frame_step(self,input_vect):
        ...
        self.gameover = font.render("GAME OVER", True, (255, 255, 255))
        ...
        # Scoring
        ballPosition = random.choice([232.5])
        if self.circle_x < 5.:
            self.bar2_score += 1
            reward = LOSE_REWARD
            self.circle_x, self.circle_y = 320., ballPosition
            self.bar1_y,self.bar_2_y = 215., 215.
        elif self.circle_x > 620.:
            self.bar1_score += 1
            reward = SCORE_REWARD
            self.circle_x, self.circle_y = 307.5, ballPosition
            self.bar1_y, self.bar2_y = 215., 215.
        ...
        if max(self.bar1_score, self.bar2_score) >= 20:
            screen.blit(self.gameover,(300.,220.))
            self.bar1_score = 0
            self.bar2_score = 0
            terminal = True
